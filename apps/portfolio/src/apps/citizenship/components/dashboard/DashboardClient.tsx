...
import { Card } from "@/apps/citizenship/components/ui/card";
...
import { GermanyMap } from '@/apps/citizenship/components/dashboard/GermanyMap';
import { LanguageSelector } from '@/apps/citizenship/components/dashboard/LanguageSelector';
...
import { RewardedAdButton } from '@/apps/citizenship/components/ads/RewardedAdButton';
import { DeleteAccountDialog } from '@/apps/citizenship/components/auth/DeleteAccountDialog';
import { AD_CONFIG } from '@/apps/citizenship/config/ads';
import { LogoutDialog } from '@/apps/citizenship/components/auth/LogoutDialog';
...
export default function DashboardClient() {
    ...
    useEffect(() => {
        ...
        if (!loading && !user) router.push('/einbuergerungstest/app/auth');
        // Check for daily streak on mount
        checkDailyStreak();
    }, [user, loading, router, checkDailyStreak]);

    // Show loading state while checking auth
    if (loading) return <div className="flex h-screen items-center justify-center bg-slate-50"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-600"></div></div>;
    ...
    return (
        <div className="min-h-screen bg-slate-50 pb-20">
                ...
                <div className="relative max-w-5xl mx-auto px-4 md:px-6 pt-8 md:pt-12 pb-20 md:pb-28">
                    ...
                    <div className="flex justify-between items-center mb-12">
                        ...
                        <div className="flex flex-col">
                            ...
                            <h1 className="text-xl md:text-3xl font-bold text-white tracking-tight drop-shadow-sm leading-tight">
                                {tHome('app_title')}
                            </h1>
                        </div>
                        ...
                        <div className="flex gap-3 items-center">
                            ...
                            <div className="hidden sm:flex items-center gap-1.5 bg-orange-500/10 border border-orange-500/20 backdrop-blur-md px-3 py-1.5 rounded-full shadow-lg shadow-orange-900/20">
                                ...
                                <span className="text-orange-100 font-bold text-sm tracking-wide">{t('day_streak', { count: dailyStreak || 0 })}</span>
                            </div>
                            ...
                            {user?.isAnonymous ? (
                                <Button
                                    onClick={() => router.push('/einbuergerungstest/app/auth')}
                                    className="bg-white/10 hover:bg-white/20 text-white border-white/20 rounded-full px-4 h-9 font-bold text-xs transition-all active:scale-95"
                                >
            ...
            <div className="relative -mt-16 md:-mt-24 max-w-5xl mx-auto px-4 md:px-6 z-20">
                <div className="bg-white/95 backdrop-blur-xl border border-white/20 text-slate-900 rounded-2xl md:rounded-3xl p-4 md:p-6 shadow-[0_20px_60px_-10px_rgba(0,0,0,0.15)] flex flex-col md:flex-row items-center gap-4 md:gap-8 justify-between relative overflow-hidden group">
                    ...
                    <div className="flex items-center gap-8 relative z-10 w-full md:w-auto">
                        {/* Circular Progress */}
                        <div className="relative h-20 w-20 md:h-28 md:w-28 flex-shrink-0 group-hover:scale-105 transition-transform duration-500">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-4 relative z-10 w-full md:w-auto mt-4 md:mt-0">
                        ...
                        {!isPremium && (
                            <Button
                                onClick={() => router.push('/einbuergerungstest/app/premium')}
            ...
            <div className="max-w-[1400px] mx-auto flex justify-center items-start">
                ...
                <main className="max-w-5xl mx-auto px-4 md:px-6 space-y-6 md:space-y-8 flex-shrink">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                            ...
                            <div
                                onClick={() => router.push('/einbuergerungstest/app/quiz?mode=exam')}
                                className="group relative overflow-hidden bg-gradient-to-br from-indigo-600 via-indigo-600 to-blue-700 rounded-2xl md:rounded-[2rem] p-5 md:p-8 text-white cursor-pointer shadow-[0_10px_30px_-10px_rgba(79,70,229,0.4)] hover:shadow-[0_20px_40px_-5px_rgba(79,70,229,0.5)] transition-all duration-300 transform hover:-translate-y-1"
                            >
                                <div className="absolute right-[-20px] top-[-20px] opacity-[0.08] group-hover:opacity-[0.15] transition-opacity duration-500">
                                    <Timer size={140} />
                                </div>
                                <div className="relative z-10">
                                    <div className="w-10 h-10 md:w-14 md:h-14 bg-white/10 backdrop-blur-md rounded-xl md:rounded-2xl flex items-center justify-center mb-4 md:mb-6 ring-1 ring-white/20 group-hover:bg-white/20 transition-all">
                                        <Timer className="h-5 w-5 md:h-7 md:w-7 text-white" />
                                    </div>
                                    <h3 className="text-lg md:text-2xl font-bold mb-1 md:mb-2 tracking-tight">{tHome('quiz_simulation')}</h3>
                                    <p className="text-indigo-100/80 text-xs md:text-sm leading-relaxed font-medium">{tHome('quiz_simulation_sub')}</p>
                                </div>
                            </div>

                            {/* My Mistakes - Premium Coral/Red Gradient */}
                            <div
                                onClick={() => router.push('/einbuergerungstest/app/quiz?mode=mistakes')}
                                className="group relative overflow-hidden bg-gradient-to-br from-rose-500 via-rose-500 to-red-600 rounded-2xl md:rounded-[2rem] p-5 md:p-8 text-white cursor-pointer shadow-[0_10px_30px_-10px_rgba(244,63,94,0.4)] hover:shadow-[0_20px_40px_-5px_rgba(244,63,94,0.5)] transition-all duration-300 transform hover:-translate-y-1"
                            >
                                <div className="absolute right-[-20px] top-[-20px] opacity-[0.08] group-hover:opacity-[0.15] transition-opacity duration-500">
                                ...
                                <div className="relative z-10">
                                    <div className="w-10 h-10 md:w-14 md:h-14 bg-white/10 backdrop-blur-md rounded-xl md:rounded-2xl flex items-center justify-center mb-4 md:mb-6 ring-1 ring-white/20 group-hover:bg-white/20 transition-all">
                                        <AlertTriangle className="h-5 w-5 md:h-7 md:w-7 text-white" />
                                    </div>
                                    <h3 className="text-lg md:text-2xl font-bold mb-1 md:mb-2 tracking-tight">{tHome('my_errors')}</h3>
                                    <p className="text-rose-100/80 text-xs md:text-sm leading-relaxed font-medium">{tHome('my_errors_sub')}</p>
                                </div>
                            </div>

                            {/* My Bookmarks - Premium Amber/Gold Gradient */}
                            <div
                                onClick={() => router.push('/einbuergerungstest/app/quiz?mode=bookmarks')}
                                className="group relative overflow-hidden bg-gradient-to-br from-amber-400 via-amber-500 to-orange-500 rounded-2xl md:rounded-[2rem] p-5 md:p-8 text-white cursor-pointer shadow-[0_10px_30px_-10px_rgba(245,158,11,0.4)] hover:shadow-[0_20px_40px_-5px_rgba(245,158,11,0.5)] transition-all duration-300 transform hover:-translate-y-1"
                            >
                                ...
                                <div className="relative z-10">
                                    ...
                                    <h3 className="text-lg md:text-2xl font-bold mb-1 md:mb-2 tracking-tight">{tHome('my_bookmarks') || "Bookmarks"}</h3>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8">
                        ...
                        <div className="lg:col-span-2">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                                {categories.map((cat) => (
                                    <div
                                        key={cat.id}
                                        onClick={() => router.push(`/einbuergerungstest/app/quiz?mode=practice&category=${cat.id}&state=${cat.isState ? selectedState : ''}`)}
                                        className="group bg-white p-4 md:p-5 rounded-xl md:rounded-[1.5rem] border border-slate-100 shadow-[0_2px_10px_rgba(0,0,0,0.03)] hover:shadow-[0_15px_30px_-5px_rgba(0,0,0,0.08)] transition-all cursor-pointer flex items-center gap-3 md:gap-5 hover:border-indigo-100 hover:bg-slate-50/50"
                                    >
                                        <div className={cn("p-3 md:p-4 rounded-xl md:rounded-2xl transition-transform duration-300 group-hover:scale-110 shadow-sm", cat.bg)}>
                                            <cat.icon className={cn("h-5 w-5 md:h-7 md:w-7", cat.color)} />
                                        </div>
                                        <div>
                                            <h4 className="font-bold text-base md:text-lg text-slate-800 group-hover:text-indigo-700 transition-colors">
                        ...
                        <div>
                            ...
                            <Card className="p-4 md:p-6 bg-white border-0 shadow-[0_20px_40px_-10px_rgba(0,0,0,0.08)] rounded-xl md:rounded-[2rem] overflow-hidden relative">
                                {/* Decorative blur */}
                                ...
                                <div className="text-center mb-6">
                                    ...
                                    <h3 className="text-xl md:text-2xl font-extrabold text-indigo-700">{selectedState}</h3>
                                </div>
